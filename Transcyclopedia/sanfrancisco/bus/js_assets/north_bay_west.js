...
santa_rosa = [];
function SantaRosaTransitCreation() {
    var santarosa_call = new XMLHttpRequest();
    santarosa_call.open("GET", "https://transit.land/api/v2/rest/routes?api_key=x5unflDSbpKEWnThyfmteM8MHxIsg3eL&operator_onestop_id=o-9qbdx-santarosacitybus&limit=700&include_geometry=true");
    santarosa_call.onreadystatechange = function() {
        if (santarosa_call.readyState === 4 && santarosa_call.status === 200) {
            var bus_route_outputs = JSON.parse(santarosa_call.responseText);
            var agency_bus = bus_route_outputs.routes[0].agency.agency_name;
            ...
            map.addSource('santa_rosa_routes', {
                'type': 'geojson',
                'data': {
                    'type': 'FeatureCollection',
                    'features': santa_rosa
            ...
            map.addLayer({
                'id': 'santa_rosa_city_bus',
                'type': 'line',
                'source': 'santa_rosa_routes',
                'layout': {
                    'line-join': 'round',
                    'line-cap': 'round'
                },
            ...
            map.on('mouseenter', 'santa_rosa_city_bus', function(e) {
                var fs = map.queryRenderedFeatures(e.point, { layers: ['santa_rosa_city_bus']});
                console.log(fs)
                ...
                if (fs.length > 0) {
                    for (var f = 0; f < fs.length; f ++) {
                        ...
                        if (hoveredPolygonLine !== null) {
                            console.log("hello!")
                            map.setFeatureState(
                                { source: 'santa_rosa_routes', id: hoveredPolygonLine },
                                { hover: false }
                            );
                        }
                        map.setFeatureState(
            ...
            map.on('mouseleave', 'santa_rosa_city_bus', (e) => {
                if (routeId.length > 0) {routeId = []}
                popup2.remove();
                // document.getElementById("range_of_routes").innerHTML = `<li class="route_radius"><span id="route_short">-</span> <span id="detailed_route">Loading...</span></li>`;
                ...
                if (hoveredPolygonLine !== null) {
                    ...
                    for (var i = 0; i < hoverIdBus.length; i++) {
                        map.setFeatureState(
                            { source: 'santa_rosa_routes', id: hoverIdBus[i]},
                            { hover: false }
                        );
                    }
                }
...
setTimeout(SantaRosaTransitCreation, 20000);