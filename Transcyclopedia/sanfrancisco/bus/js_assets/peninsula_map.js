...
function MissionBayTMACreation() {
    ...
    tma_call.onreadystatechange = function() {
        if (tma_call.readyState === 4 && tma_call.status === 200) {
            ...
            map.on('mouseenter', 'mstma_layer', function(e) {
                var fs = map.queryRenderedFeatures(e.point, { layers: ['mstma_layer']});
                console.log(fs)
                ...
                if (fs.length > 0) {
                    for (var f = 0; f < fs.length; f ++) {
                        ...
                        if (hoveredPolygonLine !== null) {
                            console.log("hello!")
                            map.setFeatureState(
                                { source: 'mstma_routes', id: hoveredPolygonLine },
                                { hover: false }
                            );
                        }
                        map.setFeatureState(
            ...
            map.on('mouseleave', 'mstma_layer', (e) => {
                if (routeId.length > 0) {routeId = []}
                popup2.remove();
                // document.getElementById("range_of_routes").innerHTML = `<li class="route_radius"><span id="route_short">-</span> <span id="detailed_route">Loading...</span></li>`;
                ...
                if (hoveredPolygonLine !== null) {
                    ...
                    for (var i = 0; i < hoverIdBus.length; i++) {
                        map.setFeatureState(
                            { source: 'mstma_routes', id: hoverIdBus[i]},
                            { hover: false }
                        );
                    }
                }
...
setTimeout(MissionBayTMACreation, 26000);